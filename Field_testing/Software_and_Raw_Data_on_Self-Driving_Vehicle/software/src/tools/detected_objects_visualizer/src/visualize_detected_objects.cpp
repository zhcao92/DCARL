...
VisualizeDetectedObjects::VisualizeDetectedObjects() : arrow_height_(0.5), label_height_(1.50)
{
...
VisualizeDetectedObjects::ObjectsToArrows(const autoware_msgs::DetectedObjectArray &in_objects)
{
  ...
  for (auto const &object: in_objects.objects)
  {
    ...
    {
      double velocity = object.velocity.linear.x*object.velocity.linear.x + object.velocity.linear.y*object.velocity.linear.y;
      ...
      if (abs(velocity) >= arrow_speed_threshold_)
      {
        ...
        double yaw = std::atan2(object.velocity.linear.y, object.velocity.linear.x);
        ...
        arrow_marker.pose.position.z = object.pose.position.z + arrow_height_;
