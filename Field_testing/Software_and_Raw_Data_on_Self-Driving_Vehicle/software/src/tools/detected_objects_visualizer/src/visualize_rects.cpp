...
VisualizeRects::VisualizeRects()
{
  ...
  detections_synchronizer_ =
    new message_filters::Synchronizer<SyncPolicyT>(SyncPolicyT(1),
...
{
  if (in_image_msg == nullptr) {
    std::cout << "in_image_msg empty" << std::endl;
  }
  if (in_objects == nullptr) {
    std::cout << "in_objects empty" << std::endl;
  }
  try
  {
    image_ = cv_bridge::toCvShare(in_image_msg, "bgr8")->image;
    cv::Mat drawn_image;
    // for (auto &object: in_objects->objects) {
    //   std::cout << "obj: " << object.x << " "    
    //             << object.y << " "
    //             << object.width << " "
    //             << object.height << " " 
    //             << object.angle << std::endl; 
    // }
...
VisualizeRects::ObjectsToRects(cv::Mat in_image, const autoware_msgs::DetectedObjectArray::ConstPtr &in_objects)
{
  ...
  for (auto const &object: in_objects->objects)
  {
    ...
    // std::cout << "sync get object" << " " <<  object.width 
    //                                 << " " <<  object.height 
    //                                 << " " <<  object.x 
    //                                 << " " <<  object.y <<  std::endl;
      ...
      double font_scale = 0.3; // 1.5;
      int thickness = 1;

      int baseline=0;
      std::string label_x = "x:" + std::to_string(object.pose.position.x);
      std::string label_y = "y:" + std::to_string(object.pose.position.y);
      std::string label_z = "z:" + std::to_string(object.pose.position.z);
      ...
      cv::Point text_origin(object.x, object.y);
      ...
      // cv::putText(final_image,
      //             label_x,
      //             text_origin + cv::Point(0, text_size.height),
      //             font_face,
      //             font_scale,
      //             cv::Scalar(50, 255, 10),
      //             thickness,
      ...
      // cv::putText(final_image,
      //             label_y,
      //             text_origin + cv::Point(0, 2*text_size.height),
      //             font_face,
      //             font_scale,
      //             cv::Scalar(50, 255, 10),
      //             thickness,
      ...
      // cv::putText(final_image,
      //             label_z,
      //             text_origin + cv::Point(0, 3*text_size.height),
      //             font_face,
      //             font_scale,
      //             cv::Scalar(50, 255, 10),
      //             thickness,
...
bool VisualizeRects::IsObjectValid(const autoware_msgs::DetectedObject &in_object)
{
    ...
    )
  {
    ...
    std::cout << "invalid obj: " <<  in_object.x 
              << " " <<  in_object.y 
              << " " <<  in_object.width 
              << " " <<  in_object.height <<  std::endl;
    return false;
  }
  return true;
}//end IsObjectValid
